...
- **You did go back into it.** The keyboard reads the field once, at that point only, and looks for which spelling is there now. Your original word standing where the fix was is a rejection, exactly as though you had backspaced. The corrected word still there is an accept.
...
- **Adaptive confidence** (on) watches how often corrections get rejected and raises the bar when a lot of them do. It stays out of the way until twenty corrections have been judged, and it can move the gate anywhere from 0.85&times; the slider (a spotless record) up to 2.5&times; it, with the result always held inside the slider's own 1.5&times;&ndash;10&times; range. It also remembers each correction you rejected: one rejected once fights uphill from then on, and one rejected twice never fires again. Undoing a correction blocks that exact pair for the rest of the session too, whatever the counts on disk say. Corrections you let stand count as well, and three of them buy back one rejection, so a pair you undid in a hurry is not sentenced for the six months the expiry clock takes. See [What counts as rejecting a correction](#what-counts-as-rejecting-a-correction).
