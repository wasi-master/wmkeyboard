...
This chip is deliberately hard to trigger, since most long digit runs are not amounts. See [Why a chip stays quiet](#why-a-chip-stays-quiet) for the rules.
...
- Chips answer what you **typed**. [Selection actions](/smart/selection-actions/) answer what you **selected**: a different bar, a different trigger, and its own switch under Advanced. The two never share the strip, because nothing is being typed while text is selected.
